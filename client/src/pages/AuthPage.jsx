...
const AuthPage = ({ onLogin }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (email && password && role) {
      onLogin(role)
      navigate(role === "citizen" ? "/citizen" : "/official")
    }
  }
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-background via-surface to-background flex items-center justify-center px-4">
      ...
      >
        <div className="bg-surface border border-muted rounded-2xl p-8 md:p-12 shadow-2xl">
          <motion.h2
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <motion.button
              type="submit"
              className="w-full py-3 bg-accent-green text-background font-bold rounded-lg hover:bg-accent-green-dark transition-all transform hover:scale-105"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
          ...
          >
            <button onClick={() => setIsSignUp(!isSignUp)} className="text-accent-green hover:underline font-semibold">
